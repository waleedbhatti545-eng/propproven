...
export default function AdminPopularPage() {
  ...
  async function fetchFirms() {
    ...
    if (data) {
      ...
      const popular = firms.filter(f => f.is_popular === true).sort((a, b) => (a.popular_order || 0) - (b.popular_order || 0));
      const available = firms.filter(f => !f.is_popular).sort((a, b) => a.name.localeCompare(b.name));
      
      setPopularFirms(popular);
      setAvailableFirms(available);
    }
    setLoading(false);
  }

  const addToPopular = (firm: FirmData) => {
    setAvailableFirms(prev => prev.filter(f => f.slug !== firm.slug));
    setPopularFirms(prev => [...prev, firm]);
  };

  const removeFromPopular = (firm: FirmData) => {
    setPopularFirms(prev => prev.filter(f => f.slug !== firm.slug));
    setAvailableFirms(prev => [...prev, firm].sort((a, b) => a.name.localeCompare(b.name)));
  };

  const moveUp = (index: number) => {
    if (index === 0) return;
    const items = [...popularFirms];
    const temp = items[index];
    items[index] = items[index - 1];
    items[index - 1] = temp;
    setPopularFirms(items);
  };

  const moveDown = (index: number) => {
    if (index === popularFirms.length - 1) return;
    const items = [...popularFirms];
    const temp = items[index];
    items[index] = items[index + 1];
    items[index + 1] = temp;
    ...
    try {
      ...
      if (availableFirms.length > 0) {
        ...
        const { error: resetErr } = await supabase.from("firms")
          .update({ is_popular: false, popular_order: 999 })
          .in("slug", availableSlugs);
          
        if (resetErr) {
            toast.error("Database Error! Did you run the SQL Migration snippet to add the popular columns?");
            setSaving(false);
            return;
        }
      }

      // 2. Set newly ordered popular firms
      const promises = popularFirms.map((firm, index) => {
        return supabase.from("firms")
          .update({ is_popular: true, popular_order: index + 1 })
          .eq("slug", firm.slug);
      });
      ...
      toast.success("Popular ranking order saved successfully!");
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 relative pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2 flex items-center gap-3">
            <Flame className="w-8 h-8 text-brand-red drop-shadow-[0_0_15px_rgba(220,38,38,0.5)] fill-brand-red/20" />
            Most Popular Controller
          </h1>
          <p className="text-neutral-400">Select exactly which firms represent the "Popular" data table view and their exact order.</p>
        </div>
        <button 
          onClick={handleSave}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {/* Active Popular Firms List */}
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse" />
                Live Popular Ordering ({popularFirms.length})
              </h2>
            </div>
              ...
              {popularFirms.length === 0 ? (
                ...
                popularFirms.map((firm, idx) => (
                  <div key={firm.slug} className="group relative bg-black/60 border border-brand-red/30 rounded-xl p-3 flex items-center gap-4 hover:border-brand-red transition-all hover:shadow-[0_0_15px_rgba(220,38,38,0.15)]">
                    ...
                    {/* Position Badge */}
                    <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center font-black text-brand-red text-sm shadow-inner group-hover:bg-brand-red/10 group-hover:border-brand-red/30 transition-colors">
                      #{idx + 1}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                       ...
                       <p className="text-xs text-neutral-400 font-mono tracking-widest">{firm.rating} RATING</p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 border-l border-white/10 pl-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <button 
                          ...
                          disabled={idx === popularFirms.length - 1}
                          className="p-1 rounded bg-white/5 text-neutral-400 hover:text-white hover:bg-white/10 disabled:opacity-20 disabled:hover:bg-white/5"
                        >
                      ...
                      <button 
                        onClick={() => removeFromPopular(firm)}
            ...
            <div className="bg-[#0c0c12]/80 border border-white/10 rounded-2xl p-4 shadow-2xl min-h-[400px] flex flex-col gap-4">
               ...
               <div className="relative group">
                 <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                   <Search className="h-4 w-4 text-neutral-500 group-focus-within:text-brand-red transition-colors" />
                 </div>
                 <input 
                   type="text"
                   ...
                   className="w-full bg-black/40 border border-white/10 rounded-lg py-2 pl-10 pr-4 text-sm text-white placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-brand-red/50 focus:border-brand-red/50"
                 />
               </div>
