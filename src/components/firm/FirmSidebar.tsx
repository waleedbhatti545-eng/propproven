...
export function FirmSidebar({
    ...
    return (
        <div className="space-y-6 sticky top-24">
            ...
            {/* Guard: If no accounts exist, show a minimal sidebar */}
            {(!firm.accounts || firm.accounts.length === 0 || !selectedAccount) ? (
                <>
                    {/* Minimal Info Card */}
                    <div className="bg-[#1A1A1A] p-6 rounded-xl border border-white/5 shadow-lg text-center space-y-3">
                        <div className="text-sm text-gray-400">No account tiers available yet.</div>
                        <div className="text-xs text-gray-500">Check back soon for pricing & challenge details.</div>
                    </div>

                    {/* 2. DISCOUNT CARD */}
                    {firm.discountCode && (
                        <div className="bg-gradient-to-br from-brand-red to-brand-orange p-6 rounded-xl border border-brand-red shadow-xl relative overflow-hidden group">
                    ...
                    <div className="bg-[#1A1A1A] p-6 rounded-xl border border-white/5 space-y-4 text-center">
                        <Button className="w-full bg-brand-red hover:bg-brand-orange text-white font-bold h-12 rounded-lg shadow-lg gap-2">
                            Visit {firm.name} <ExternalLink className="w-4 h-4" />
                        </Button>
                ...
                <>
                    ...
                    <div className="bg-[#1A1A1A] p-6 rounded-xl border border-white/5 shadow-lg relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-end mb-6 border-b border-white/5 pb-4">
                            ...
                            {selectedAccount.promoPrice && (
                                <div className="text-right">
                                    <div className="text-xs text-brand-red font-bold uppercase bg-brand-red/10 px-2 py-1 rounded mb-1">Sale</div>
                        ...
                        <div className="space-y-3 text-sm">
                            <StatRow label="Target" value={`${selectedAccount.targetPhase1 || 'N/A'} / ${selectedAccount.targetPhase2 || 'N/A'}`} highlight />
