...
export function SectionCommissions({ firm }: { firm: FirmData }) {
    ...
    return (
        <div className="relative pt-10 border-t border-white/5">
            <div className="absolute top-[20%] right-[30%] w-[400px] h-[200px] bg-brand-red/5 blur-[120px] rounded-full pointer-events-none" />
            ...
            <div className="bg-[#111111]/80 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden relative group transition-all duration-500 shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:border-brand-red/30 z-10 max-w-4xl">
                ...
                <div className="divide-y divide-white/5 relative z-10">
                    {data.map((item) => (
                        <div key={item.asset} className="grid grid-cols-2 p-5 md:p-6 hover:bg-brand-red/5 transition-all duration-300 group/row border-l-2 border-transparent hover:border-brand-red cursor-default bg-black/20">
                            <div className="text-white font-medium flex items-center gap-4 transition-transform duration-300 group-hover/row:translate-x-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-white/20 group-hover/row:bg-brand-red group-hover/row:shadow-[0_0_10px_rgba(255,0,0,0.8)] transition-all duration-300" />
                                <span className="group-hover/row:text-brand-red transition-colors">{item.asset}</span>
                            </div>
            ...
            <p className="mt-5 text-sm font-medium text-gray-500 flex items-center gap-2 relative z-10 opacity-70 max-w-4xl">
                <span className="text-brand-red">*</span> Commissions are per lot round turn unless specified otherwise.
            </p>
        </div>
    );
}
